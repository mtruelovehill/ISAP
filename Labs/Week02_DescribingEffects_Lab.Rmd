...
pacman::p_load('tidyverse', 'rstatix', 'shiny', 'kableExtra')
...
output$effectSizePlot <- renderPlot({
  ...
  ggplot(data.frame(x=c(15, 85)), aes(x)) +
    geom_histogram(data = testDat[testDat$Group=='G1',], aes(x, y = after_stat(density)), binwidth = 2, fill = accent2, alpha = .6) +
    geom_histogram(data = testDat[testDat$Group=='G2',], aes(x, y = after_stat(density)), binwidth = 2, fill = accent1, alpha = .6) +
    geom_density(data = testDat[testDat$Group=='G1',], aes(x), color = accent2, linewidth = 1) +
    geom_density(data = testDat[testDat$Group=='G2',], aes(x), color = accent1, linewidth = 1) +
  ...
  observeEvent(input$do, {
    ...
    CIs$plotColors <- ifelse(mean(ciDat$Cscore)>=CIs$lowBound&mean(ciDat$Cscore)<=CIs$hiBound, accent2, accent1)
...
```{r, echo = F}
tags$style(HTML(".js-irs-0 .irs-single, .js-irs-0 .irs-bar {background: #DFAA73; border-color: transparent}"))
```
...
fluidRow(column(6,
                ...
                selectInput('n', 'Sample Size per Group:', selected = '100',
...
ggplot(ciDat, aes(Cscore)) + geom_histogram(fill = accent1, color = baseColor) + 
  xlab('Score') +
  theme(axis.text.x = element_text(size = 12),
          axis.title.x = element_text(size = 14, face = 'bold'),
          axis.text.y = element_blank(),
...
(powPlot <- ggplot(powerDat, aes(SampleSize, Power, color = EffectSize)) + geom_point() +
  ...
  scale_color_manual(values = c(baseColor, accent1, accent3), labels = c('Strong', 'Moderate', 'Weak')) +
...
powPlot + 
  geom_segment(x = 0, xend = 63, y = .8, yend = .8, linetype = 'dashed', color = accent2) +
  geom_segment(x = 63, xend = 63, y = 0, yend = .8, linetype = 'dashed', color = accent2) +
  annotate(geom = 'point', x = 63, y = .8, shape = 8, size = 3, color = accent2)
```
...
```{r, echo = F, fig.width=6, fig.height=3, fig.align='center'}
powPlot + 
  geom_segment(x = 0, xend = 50, y = .16, yend = .16, linetype = 'dashed', color = accent2) +
  geom_segment(x = 50, xend = 50, y = 0, yend = .16, linetype = 'dashed', color = accent2) +
  annotate(geom = 'point', x = 50, y = .16, shape = 8, size = 3, color = accent2)
```
...
This week, you'll be working with data collected from RMAP students from previous years. You'll use these data to generate descriptive statistics and visualisations. Specifically, this dataset contains the following variables:



```{r, echo = F}
dat <- read.csv('https://mtruelovehill.github.io/ISAP/Labs/Week2LabData.csv')
datInfo <- data.frame(VariableName=colnames(dat), 
                      Description=c('Participant ID', 'Levels: 18-33; 34-48; 49-64', 
                                    'Sex assigned at birth; Levels: Female; Male',
                                    'Current gender; Levels: Male; Female; Non-binary/Third Gender; Prefer not to say', 
                                    'Participant ethnicity', 
                                    'Highest level of education completed',
                                    'Total Perfectionism score',
                                    'Total Self-Construal well-being score',
                                    'Total Aggression Questionnaire score'))

datInfo %>%
  kbl(col.names=c('Variable Name', 'Description')) %>%
  kable_styling(full_width = F) %>%
  row_spec(0, bold = T, color=accent2, font_size = 18, align='l') %>%
...
quiz(caption = 'Check Your Understanding',
     question('Which of the following "Measure" values used by SPSS reflects categorical data? Please select all that apply.',
              ...
              answer('Scale'),
              message = 'Scale can be used for both types of continuous data (ratio and interval scales of measurement), but categorical variables are split into nominal and ordinal measures. Remember that nominal data reflect purely independent categories, but ordinal data have some sort of implied order (e.g. Likert Scale data, level of education).'))
```
...
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/spss_varView.png')
```
...
**Ordinal:** data that fall into discrete categories that have some kind of meaningful ordering information (e.g. Likert Scale responses)
**Scale:** continuous/numerical data; data have some kind of numeric relationship and could technically be measured to infinite decimal places, if the measurement was capable of doing so.


To adjust the measure type, select the Measure box for a single variable and click the arrow that appears.

* `ID` is represented as a number, but there really is no numeric relationship between the numbers (e.g. someone with an ID = 40 didn't complete the study in twice the time of someone with an ID = 20). Technically, the IDs are ordered based on when they completed the study, but because this information isn't meaningful in the context of our analysis, this is considered a 'Nominal' variable.

* `AgeBracket` is ordinal. Because age was measured as brackets rather than raw numbers, it cannot be considered continuous. However, the brackets do give us a sense of the order; someone in the first age bracket is younger than those in the second and third brackets. 

* `SAB`, `Gender`, `Ethnicity`, and `Education` are all nominal. They can be captured in discrete categories and there is no inherent order.

* `Perfectionism`, `SelfConstrual` and `ACG` are all 'Scale' variables. There is a meaningful numeric relationship between the values. 
...
This should also be done in *Variable View*. The column names may seem a bit misleading; you won’t use the ‘Label’ column for this task, but the 'Values' column.
...
To associate a label with each value of a variable, you'll use the **Values** column under the *Variable View* tab. Select the Gender x Values cell and click the dots to bring up the *Value Labels* box. 
...
```{r, echo = F, out.width='75%', fig.align='center'}
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/spss_valueLabs.png')
```
...
+ [ ] $\ $ Check the distribution of your categorical variables using a frequency table and/or a figure
...
ggplot(freqDat, aes(Bar)) + geom_bar(fill = accent1, color=baseColor) +
  labs(x ='Bar Chart') +
...
ggplot(freqDat, aes(Histogram)) + geom_histogram(fill = accent1, color=baseColor) +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title.y = element_blank(),
        axis.title.x = element_text(size = 14, face = 'bold'))
...
quiz(caption = 'Check Your Results',
     question_numeric("How many datapoints do you have for the gender variable?",
              answer(251, correct=T),
              message = 'There are 251 participants with gender data. Although there are 373 participants total, 122 of them are missing gender data'),
     question_numeric("How many participants in the sample have a PhD or other professional qualification?",
              answer(6, correct=T),
              message = 'There are 6 participants with a PhD or other professional degree in this sample.'),
     ...
     question('True or False: No participants scored above 175 on the self-construal scale.',
              answer('True'),
              answer('False', correct = T),
              message = 'The histogram and maximum value indicates that at least one participant achieved a score higher than 175')
     )
```
...
Select *Analyze>Descriptives>Frequencies*. Select `AgeBracket`,`SAB`, `Gender`, `Ethnicity`, and `Education` and use the arrow to move them into the Variable box. Make sure 'Display frequency tables' is checked. To also produce a bar chart, select 'Charts...' and choose 'Bar charts.'

</br>

```{r, echo = F, out.width='75%', fig.align= 'center'}
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/spss_freqTable.png')
```

The output shows the number of participants in each category, as well as the proportions and total values overall. If you added the labels for `Gender` properly, the table will use them rather than the original numbered data, allowing for ease of interpretation.

```{r, echo = F, out.width='75%', fig.align= 'center'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/spss_freqOutput.png')
```
...
Select *Analyze>Descriptives>Frequencies*. Select `Perfectionism`, `SelfConstrual`, and `AGQ` and move them into the 'Variable' box. Uncheck 'Display frequency tables' unless you want to produce massive, unnecessary tables. To produce histograms, select 'Charts...' and choose 'Histograms'. You'll get a histogram for each variable:

```{r, echo = F, out.width='75%', fig.align= 'center'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/spss_histogram.png')
```
...
<div class='nobullet'>
+ [ ] $\ $ Check the appropriate measure of central tendency for ordinal and continuous
+ [ ] $\ $ Check the standard deviation of continuous variables
+ [ ] $\ $ Check the minimum and maximum values of continuous variables
...
quiz(caption = 'Check Your Results',
     ...
     question_numeric("What is the average score on the Aggression Questionnaire? Please round your answer to two decimal places.",
              answer(87.90, correct=T),
              message = 'The average social perception scale score is 87.90.'),
     question_numeric("What is the lowest perfectionism score in this sample?",
              answer(16, correct=T),
              message = 'The lowest perfectionism score in this sample is 16.'),
     question_numeric("What is the standard deviation of the self-construal variable? Please round your answer to two decimal places.",
              answer(12.79, correct=T),
              message = 'The standard deviation of the age variable is 12.79.')
)
```
...
Use *Analyze>Descriptive Statistics>Frequencies* to check your descriptives. If you need a refresh on measures of central tendency and standard deviation, see [these slides](https://mtruelovehill.github.io/PRM/Lectures/Week01_ExploringData_Lecture.html).
...
Note that ordinal variables should only be described using either mode or median. In the case of `AgeBracket`, mode is most appropriate. However, ordinal likert-scale variable can be described using their median. 

```{r, echo = F, out.width='75%', fig.align='center'}
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/spss_catDesc.png')
```

The continuous variables can be described using all 3 measures of central tendency, but as long as they are normally distributed, mean is almost always the most appropriate and useful. SD, minimum, and maximum can also be checked here.

```{r, echo = F, out.width='75%', fig.align='center'}
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/spss_contDesc.png')
```
...
Now that you've reviewed your data: 

<div class='nobullet'>

+ [ ] $\ $ Write a one or two APA-style sentences describing your sample that you could incorporate into a methods section. 
+ [ ] $\ $ Create an APA-style table to present the descriptive statistics of the other variables in the dataset. 

</div>

When describing your sample, you should **at least** include the following: 

+ $n$ of study
+ Age (minimum, maximum, mean, standard deviation; if age was measured categorically, $n$ in each category)
+ Gender ($n$ in each category)

If you are working with data that includes other demographic variables such as race or education, these should be appropriately described as well. You would also include any exclusion/inclusion criteria (although you don't need to do that in this case, as you don't know anything about the study!).

Descriptive statistics of the other variables should also be reported in the results section.

+ $n$ in each level of a categorical variable 
+ Mean and SD for continuous variables

If there are several variables, this is usually presented in table format, alongside the relevant statistical analysis values. However, they may also be added directly into the text if there are only a few variables.
...
You can find an example of APA-style tables with descriptive statistics included [here](https://apastyle.apa.org/style-grammar-guidelines/tables-figures/sample-tables#demographic).
