...
**You can download the dataset from the first experiment [here.](https://mtruelovehill.github.io/ISAP/Labs/Week3LabData.sav)** This dataset contains the following variables:
...
datInfo <- data.frame(VariableName=colnames(dat),
                      Description=c('Levels: M = Male, F = Female', 'Infant age in weeks',
                                    ...
                                    'Bonus content: repeated-measures buggy vocalisation time',
                                    'Bonus content: repeated-measures backpack vocalisation time'))
...
While in *Variable View*, add a label to `Transport` by clicking the associated row in the 'Values' column. Put '0' in the Value column and 'Buggy' in the Label column, click '+', then repeat the process for 'Backpack'.
...
Note that the numbers inside the curly brackets refer to the degrees of freedom. Please refer to the [lecture slides](https://mtruelovehill.github.io/ISAP/Lectures/Week03_ttests_lecture.html) for a specific example.
...
You can follow the steps from the [Descriptive Data] tab
...
You've already checked the demographic variables. The only new variables are `RM_buggy` and `RM_backpack`. To check the descriptives of these continuous variables, click *Analyze>Descriptive Statistics>Frequencies*, add `RM_buggy` and `RM_backpack` to the Variable(s) box. Make sure to untick the 'Display Frequency Tables' box. Click 'Statistics' and then click 'Mean', 'Std. Deviation', 'Minimum', and 'Maximum'. Click 'Continue' and then 'OK'. 
...
```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/week3_diffScore.png')
```

Perform the normality tests from [Checking Assumptions] with this new variable.

```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/week3_diffScoreHist.png')
```

```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/PRM/Labs/images/week3_diffScoreQQ.png')
```
...
### Task 4
+ [ ] $\ $ Run a paired-samples $t$-test to test your hypothesis; make sure to also check the effect size and 95% confidence interval
...
```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/week3_pairedT.png')
```
...
```{r, echo = F, fig.align= 'center', out.width='75%'}
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/week3_pairedES.png')
```
...
See the hint from [Intepret & Report].
...
Our sample consisted of 36 infants between 21 and 45 weeks old ($M$ = 33.13, $SD$ = 5.48; 15 females, 21 males). Assumptions checks were conducted through visual assessment. All data met the assumptions of normality and independence.

A paired-samples $t$-test indicated that transportation modality had a significant effect on infant vocalisation, $t$(35) = -2.30, $p$ = .027, 95% CI[-2.35, -0.15], $d$ = -0.38. Specifically, infants vocalised significantly more while riding in a backpack ($M$ = 15.07, $SD$ = 3.65) than when riding in a buggy ($M$ = 13.82, $SD$ = 3.45).
...
+ [ ] $\ $ For the within-subjects version of the experiment, imagine that you already had data from 40 participants recruited as part of a larger study. You want to determine the effect size this sample has the power to detect, given an $\alpha$ = .05 and a power = .8. Run the appropriate power analysis. 
...
quiz(caption = 'Test Your Understanding',
     question("Recall that there were 18 participants per group in this dataset. True or False: The current analysis is adequately powered to detect a moderate effect size (d = .50), given power = .8 and alpha = .05",
              answer("True"),
              answer("False", correct = T),
              message = 'False; This study only has 18 participants per group. If the effect of transport modality on infant vocalisation is weak or moderate, a Type II error is likely to occur.'),
