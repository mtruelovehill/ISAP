...
output$aucPlot <- renderPlot({
  ...
  ggplot(data.frame(x=c(-6, 6)), aes(x)) + 
    ...
    geom_area(stat = "function", fun = dnorm, fill = accent1, xlim = c(qnorm(loLim), qnorm(hiLim)),
              alpha = .7) + 
...
output$aucScorePlot <- renderPlot({
  ...
  ggplot(data.frame(x=c(31, 84)), aes(x)) +
    stat_function(fun=dnorm, geom = "line", linewidth = 1.5, args = c(mean = 58, sd = 9), color = accent2) +
    geom_area(stat = "function", fun = dnorm, fill = accent1, args = c(mean = 58, sd = 9), xlim = c(loLim, hiLim),
              alpha = .7) +
    theme(axis.text = element_blank(),
          axis.ticks = element_blank(),
          axis.title = element_blank()) +
...
output$alphaPlot <- renderPlot({
  ggplot(data.frame(x=c(-5, 5)), aes(x)) +
    stat_function(fun=dnorm, geom = "line", linewidth = 1.5, args = c(mean = 0, sd = 1), color = accent2) + 
...
output$pPlot <- renderPlot({
  ...
  ggplot(data.frame(x=c(-5, 5)), aes(x)) +
    ...
    xlab('test statistic') +
    geom_vline(xintercept = input$testStatistic, color = baseColor, linewidth = 1.5, linetype='dashed') +
    theme(axis.text.y = element_blank(),
          axis.ticks.y = element_blank(),
          axis.title.y = element_blank(),
          axis.text.x = element_text(size = 14, face = 'bold'),
...
output$testPlot <- renderPlot({
  ...
  ggplot(data.frame(x=c(-5, 5)), aes(x)) +
    ...
    scale_x_continuous(breaks = seq(-5, 5, by = 1)) +
    xlab('test statistic') +
    geom_vline(xintercept = input$testStat2, color = baseColor, linewidth = 1.5, linetype='dashed') +
    theme(axis.text.y = element_blank(),
          axis.ticks.y = element_blank(),
          axis.title.y = element_blank(),
          axis.text.x = element_text(size = 14, face = 'bold'),
...
```{r, echo = F}
tags$style(HTML(".js-irs-0 .irs-from, .js-irs-0 .irs-to, .js-irs-0 .irs-bar {background: #DFAA73; border-color: transparent}"))
tags$style(HTML(".js-irs-1 .irs-from, .js-irs-1 .irs-to, .js-irs-1 .irs-bar {background: #DFAA73; border-color: transparent}"))
tags$style(HTML(".js-irs-2 .irs-single, .js-irs-2 .irs-bar {background: #DFAA73; border-color: transparent}"))
tags$style(HTML(".js-irs-3 .irs-single, .js-irs-3 .irs-bar {background: #DFAA73; border-color: transparent}"))
```
