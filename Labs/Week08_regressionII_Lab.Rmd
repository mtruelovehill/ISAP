...
**NOTE:** If you use the dataset from last week, you'll need to update the variable names, as PROCESS doesn't like names longer than 8 characters.
...
These are only the hypotheses associated with the interaction; if your research objectives reference the marginal effects, you should include those as well.
...
<div class="nobullet">
+ [ ] $\ $ Download the PROCESS macro from [this link](https://haskayne.ucalgary.ca/CCRAM/resource-hub)
...
Find the 'processv5' folder in your Downloads folder and move it to your Documents folder or somewhere more permanent. 

Navigate to *Extensions > Utilities > Install Local Extension Bundle*. Navigate to the Process 5 folder on your device, and click the *PROCESS v5.0  for SPSS* and then 'process.spe'. It should install and give a message letting your know it's done. Now, you'll be able to use PROCESS in SPSS. 
...
quiz(caption = 'Check Your Results',
     ...
     question("Which of the following interpretations are correct? Please select all that apply.",
              answer('For every additional hour of sleep a participant gets per week, their well-being score will increase by 0.37, when social support is held constant and one has an average level of physical activity', correct=T),
...
To perform a linear regression, navigate to *Analyze > Regression > PROCESS v5.0...*.
...
Set 'Model' to 1 to indicate that you want to perform moderation.

```{r, echo = F, fig.align= 'center', out.width = '50%'}
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/regressionII_processMod.png')
```

Click 'Options'. Under 'Mean center for construction of products', tick 'Only continuous components.' This means that our variables that were multiplied for our interaction will be mean centered, while `SocSupp` will remain the same. This also helps to address the multicollinearity issue. Select to probe interactions in cases where $p$ < .05, click 'Johnson-Neyman technique', and under 'Values for nondiscrete moderators', click 'Moments'. You can also request descriptives and correlations and the adjusted R-squared value.

```{r, echo = F, fig.align= 'center', out.width = '50%'}
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/regressionII_processOptions.png')
```

Click 'OK', then 'Run'. 

The first piece of output is the test of the overall model. 

```{r, echo = F, fig.align= 'center', out.width = '50%'}
knitr::include_graphics('https://mtruelovehill.github.io/ISAP/Labs/images/regressionII_PrimaryModelResults.png')
```
