...
+ We have already investigated categorical $\times$ categorical interactions:
  + The effect of a <span style = "color:#BB5A73"> <b> categorical </span> </b> IV on the DV changes at different levels of a second <span style = "color:#BB5A73"> <b> categorical </span> </b> IV
]
...
+ We'll expand this to continuous $\times$ continuous interactions:
  ...
  + The slope of the model line between a <span style = "color:#BB5A73"> <b> continuous </span> </b> predictor and the outcome changes as the values of a second <span style = "color:#BB5A73"> <b> continuous </span> </b> predictor change.
...
$$y_i = \beta_0 + \beta_1x_i + \beta_2z_i + \beta_3\color{#BB5A73}{xz_i} + \epsilon_i$$
...
+ To download, please [click this link](https://haskayne.ucalgary.ca/CCRAM/resource-hub)
...
+ **Step 3:** Navigate to *Analyze > Regression > PROCESS v#.# by Andrew F. Hayes*

+ **Step 4:** Set up your model & run it

+ Assuming you've already downloaded PROCESS macro from [this link](https://haskayne.ucalgary.ca/CCRAM/resource-hub), let's continue in SPSS
...
In this study, we investigated the association between years of service and job performance on someone's salary. We also intended to investigate whether there was a moderating effect of job performance on the association between years of service and salary. To test this, we performed multiple linear regression with salary as the outcome variable and years of service and job performance as predictors. We also included an interaction between years of service and job performance. $\alpha$ was set at .05 for all analyses.

The normality assumption was checked through visual assessment of a P-P plot of the residuals. Linearity and homoskedasticity were checked using a residuals by predicted values plot. The data met all assumptions. VIF scores and correlations were used to evaluate the presence of multicollinearity. The presence of an interaction term introduced multicollinearity, so mean-centered versions of the years of service and job performance variables were used in the analysis. 

---
### Reporting Results

Our results indicate that the overall model significantly predicted well-being, $F$(3,96) = 37.99, $p$ < .001, explaining 53% ($R^2_{adj}$) of the variance in student well-being scores. There was a marginal effect of job performance on salary; for every one-unit increase in job performance, salary increased by £971, $p$ < .001, 95% CI = [0.79, 1.16] in those with an average number of years of service. There was no significant association between years of service and salary, $\beta$ = 1.39, $p$ = .309, 95% CI = [-1.31, 4.09]. 


There was a significant interaction between job performance and years of service on salary, $\beta$ = .21, $p$ = .008, 95% CI = [0.06, 0.37]. A simple-slopes analysis revealed that while years of service was not associated with salary in those with generally lower than average (< -1 $SD$; $p$ = .160) or average job performance ($p$ = .309), this was not the case in those with higher (> +1 SD) job performance. In this group, as years of service increased by one year, salary increased by £5427 pounds, $SE$ = 2.16, $p$ = .014, 95% CI = [1.13, 9.72].

The Johnson-Neyman analysis indicated that those who scored 30.68 points below the average level of job performance and those who scored 8.28 points above the average job performance had significant associations between years of service and salary.
