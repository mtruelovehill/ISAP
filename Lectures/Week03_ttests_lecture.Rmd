...
** Are the groups that produced these means different? (Yes/No/It Depends)**
...
+ Generally used to test the difference between two means
  ...
  + **One-sample $t$-tests** test the difference between a sample mean and a known value
...
.pull-right[
+ **One-sample $t$-test:** we estimate the <span style = "color:#BB5A73"> mean of our data </span>
    + $df = n - 1$
  
+ **Independent samples $t$-test:** we estimate the <span style = "color:#BB5A73"> mean of each of our two groups </span>
    + $df = n - 2$
  
+ **Paired-samples $t$-test:** we estimate the <span style = "color:#BB5A73"> mean difference between observations </span>
    + $df = n - 1$
]
...
```{r, echo = F, fig.height=4}
ggplot(data = data.frame(t = c(-5, 5)), aes(x=t)) +
  stat_function(fun=dt, geom='line', args=c(df = 15), color = accent1, linewidth=1.5) +
  geom_vline(xintercept = 2.86, color = accent2, linetype = 'dashed', linewidth = 1.5) +
...
ggplot(data = data.frame(t = c(-5, 5)), aes(x=t)) +
  ...
  annotate('text', x = 3.25, y = .05, parse = T, label = paste('alpha == .05'), size = 4, color = accent1)
```
...
ggplot(data = data.frame(t = c(-5, 5)), aes(x=t)) +  
  ...
  annotate('text', x = 3.25, y = .05, parse = T, label = paste('alpha == .025'), size = 6, color = accent1) +
  annotate('text', x = -3.25, y = .05, parse = T, label = paste('alpha == .025'), size = 6, color = accent1)
```
]
...
ggplot(assumpDat, aes(normDat)) + geom_histogram(fill = baseColor, color = accent2, bins = 30) +
...
ggplot(assumpDat, aes(bimodDat)) + geom_histogram(fill = baseColor, color = accent2, bins = 30) +
  labs(x='Score', y = 'Count') +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 14, face = 'bold'))

ggplot(assumpDat, aes(floorDat)) + geom_histogram(fill = baseColor, color = accent2, bins = 30) +
  labs(x='Score', y = 'Count') +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 14, face = 'bold'))
```
...
+ Let's do this in SPSS using [these data](https://mtruelovehill.github.io/ISAP/Data/BPdat.sav).
...
+ Normality
  ...
  + Do <b> not </b> run tests of normality; these may not give accurate results.
...
+ Homogeneity of Variance
  ...
  + Some suggest using Levene's test; however, this may be problematic (see Delacre et al., 2017)
  
  + Instead, default to Welch's t-test
...
ggplot(data = data.frame(t = c(-5, 5)), aes(x=t)) +
  stat_function(fun=dt, geom='line', args=c(df = 128), color = accent3, linewidth=1.5) +
  ...
  theme(axis.text.y = element_blank(),
        ...
        axis.text.x = element_text(size = 14),
        axis.title.x = element_text(size = 16, face = 'bold')) +
  annotate('text', x = -4.25, y = .05, label = 't = -3.16', size = 8, color = baseColor) +
  geom_vline(xintercept = -3.16, color = baseColor, linetype = 'dashed', linewidth = 1.5)
...
We conducted a **two-tailed independent-samples $t$-test** to determine the effect of <span style = "color:#DFAA73"> <b> treatment </span></b> on <span style = "color:#DFAA73"> <b>systolic blood pressure (SBP) </span></b>. There was a significant effect of treatment, <span style = "color:#BB5A73"><b> $t$(128) = -3.16, $p$ = .002, 95% CI = [-4.68, -1.08], $d$ = -.55 </span></b>. Specifically, SBP in the treatment group was lower <span style = "color:#F0D264"><b> ( $n$ = 65, $M$ = 136.97, $SD$ = 4.86)</span></b> than SBP in the placebo group <span style = "color:#F0D264"><b>( $n$ = 65, $M$ = 139.85, $SD$ = 5.50)</span></b>.
...
+ Research Question: Does Medication X affect blood pressure in participants over time?
...
+ Data used for this example can be found [here](http://mtruelovehill.github.io/ISAP/Data/BPpairedDat.sav)

+ As before, descriptive statistics and relevant plots should be generated.
...
.pull-left[
+ Normality of <span style = "color:#DFAA73"><b>difference scores </span></b>
...
+ Independence (of participants rather than observations)
  ...
  + Consider study design; are there any reasons that one participant's scores may impact another's?
]

.pull-right[

```{r, echo = F}
knitr::include_graphics('images/diffScore.png')
```
...
knitr::include_graphics('images/diffHistogram.png')
```


```{r, echo = F, out.width='60%'}
knitr::include_graphics('images/diffQQ.png')
```
...
knitr::include_graphics('images/pairedtTest.png')
```
...
ggplot(data = data.frame(t = c(-7, 7)), aes(x=t)) +
  stat_function(fun=dt, geom='line', args=c(df = 34), color = accent3, linewidth=1.5) +
  ...
  theme(axis.text.y = element_blank(),
        ...
        axis.text.x = element_text(size = 14),
        axis.title.x = element_text(size = 16, face = 'bold')) +
  annotate('text', x = -4.75, y = .05, label = 't = -6.32', size = 8, color = baseColor) +
  geom_vline(xintercept = -6.32, color = baseColor, linetype = 'dashed', linewidth = 1.5)
...
We conducted a **two-tailed paired-samples $t$-test** to determine the effect of <span style = "color:#DFAA73"> <b> treatment  </span></b> on <span style = "color:#DFAA73"> <b>systolic blood pressure (SBP) </span></b>. There was a significant effect of treatment, <span style = "color:#BB5A73"><b> $t$(128) = -6.32, $p$ < .001, 95% CI = [-12.05, -6.18], $d$ = -1.07 </span></b>. Specifically, SBP was lower before the medication was administered <span style = "color:#F0D264"><b> ( $M$ = 138.60, $SD$ = 6.67)</span></b> than after taking the medication <span style = "color:#F0D264"><b>( $M$ = 147.71, $SD$ = 8.48)</span></b>.
