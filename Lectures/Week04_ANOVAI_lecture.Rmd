...
+ ANOVAs identify differences in means across multiple (3+) groups
...
### Types of ANOVA
  ...
  + **One-Way ANOVA** test for differences between 3 or more mean values
...
<b><p style="color:#BB5A73;">Example research question:</b></p>
...
+ To determine the variation in the DV explained by the IV ($MS_{model}$), we need to understand how much better our IV model does compared to the mean model.
...
.pull-left[
$$F = \frac{\color{#BB5A73}{MS_{model}}}{MS_{residual}}$$

$MS_{model} = \frac{\color{#BB5A73}{SS_{model}}}{df_{model}}$
...
$MS_{model} = \frac{\color{#BB5A73}{`r sum(dat$sqGMDevs)`}}{df_{model}}$
...
$MS_{model} = \frac{`r sum(dat$sqGMDevs)`}{\color{#BB5A73}{2}}$
...
.pull-left[
$$F = \frac{\color{#BB5A73}{`r sum(dat$sqGMDevs)/2`}}{MS_{residual}}$$
...
.pull-left[
$$F = \frac{`r sum(dat$sqGMDevs)/2`}{\color{#BB5A73}{MS_{residual}}}$$


$MS_{residual} = \frac{\color{#BB5A73}{SS_{residual}}}{df_{residual}}$
...
$MS_{residual} = \frac{\color{#BB5A73}{`r sum(dat$sqDevs)`}}{df_{residual}}$
...
$MS_{residual} = \frac{`r sum(dat$sqDevs)`}{\color{#BB5A73}{9}}$
...
.pull-left[
$$F = \frac{`r sum(dat$sqGMDevs)/2`}{\color{#BB5A73}{`r round(sum(dat$sqDevs)/9, 2)`}}$$
...
1) An $F$-statistic is calculated

2) The null distribution based on $df_{model}$ and $df_{residual}$ is identified 

3) Using this distribution, the probability of getting an $F$-statistic at least as extreme as yours is computed
...
3. Check the data (visualisations/descriptives)
...
+ $H_0: \mu_1 = \mu_2 =...\mu_n$
  ...
  + $n$ = the total number of levels in the IV

+ $H_1:$ at least one $\mu$ is different from the other $\mu$s

+ Note that the ANOVA distribution only has one tail so is inherently a one-tailed test

  + This is due to the nature of the $F$-statistic calculation (a ratio cannot be less than 0)

+ However, $H_1$ is still non-directional
  + ANOVA results don't provide information about the direction of group differences, just that (at least) one exists.
...
**Example Research Objective:** Determine whether the type of auditory stimulus an infant is exposed to has an effect on their affect

  + A caution about causal language...
...
+ Check the sample required to achieve 80% power to detect a moderate effect ( $eta^2$ = .06) with $\alpha$ = .05.

+ Although $\eta^2$ is produced in SPSS and commonly reported, Webpower requires the effect size to be entered as $f$, so it must be converted using the following formula:
...
+ Check relevant plots
  
--

+ Let's continue in SPSS using [these data](https://mtruelovehill.github.io/PRM/Data/babyDat.sav).
...
Although reports will vary depending on the study, the following should be reported at minimum:
...
+ Descriptive data for each group ( $M$ & $SD$)
...
We conducted a **One-Way ANOVA** to determine the effect of <span style = "color:#BB5A73"> <b> auditory stimulus </span></b> on <span style = "color:#BB5A73"> <b> an infant's affect</span></b>. The $\alpha$ threshold was set at .05 for all analyses. There was a significant effect of stimulus, <span style = "color:#CD826E"><b> $F$(2, 57) = 5.99, $p$ = .004, $\eta^2$ = .174, 95% CI = [.02, .33] </span></b>. Posthoc Tukey's tests indicated that infants listening to infant-directed singing displayed a positive or neutral affect significantly longer <span style = "color:#DFAA73"><b> ( $M$ = 6.32, $SD$ = 2.27)</span></b> than infants listening to adult-directed speech <span style = "color:#DFAA73"><b> ( $M$ = 4.36, $SD$ = 1.55)</span></b>, <span style = "color:#F0D264"><b> $p$ = .005,</span></b> or infants listening to infant-directed speech <span style = "color:#DFAA73"><b>( $M$ = 4.78, $SD$ = 1.77)</span></b>, <span style = "color:#F0D264"><b> $p$ = .033.</span></b> There was no significant difference in time of distress in infants listening to adult-directed speech and those listening to infant-directed speech, <span style = "color:#F0D264"><b> $p$ = .762. </span></b> 
...
.pull-left[.center[<span style = "color: #CD826E"> Null Hypothesis </span>
$$\mu_{parentAbsent} = \mu_{siblingPresent} = \mu_{parentPresent}$$
]]

.pull-right[.center[<span style = "color: #CD826E"> Alternative Hypothesis </span>
...
knitr::include_graphics('images/sphericity.png')
```
]
...
We conducted a **Repeated-measures one-way ANOVA** to determine the effect of <span style = "color:#CD826E"> <b> familial presence  </span></b> on <span style = "color:#CD826E"> <b>infant affect </span></b>. There was a significant effect of familial presence, <span style = "color:#CD826E"><b> $F$(1.83, 107.99) = 42.40, $p$ < .001, $\eta^2_p$ = 0.42 </span></b>. Mean comparisons with Bonferroni corrections indicated that infants became distressed more quickly when no family members were present <span style = "color:#DFAA73"><b> ( $M$ = 3.85, $SD$ = 2.05)</span></b> than when a sibling was present <span style = "color:#DFAA73"><b>( $M$ = 5.04, $SD$ = 1.79)</span></b>, <span style = "color:#F0D264"><b> $p$ < .001,</span></b> and when a parent was present <span style = "color:#DFAA73"><b>( $M$ = 7.03, $SD$ = 2.05)</span></b>,<span style = "color:#F0D264"><b> $p$ < .001.</span></b> Infants retained a positive or neutral affect for longer when a parent was present than when a sibling was present,<span style = "color:#F0D264"><b> $p$ < .001.</span></b> 
