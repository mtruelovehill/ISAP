...
+ ANOVAs can be used to compare more than two means. 
  ...
  + One-Way ANOVA tests for mean differences in the dependent variable between 3 or more levels of a single independent variable. 
  
  + **Factorial (Multiway) ANOVA** tests for mean differences in the dependent variable across <b>multiple</b> independent variables
...
+ Factorial ANOVAs test for mean differences in the dependent variable across <b>multiple</b> independent variables
...
+ When using factorial ANOVA, you can test for both main effects and interactions
  ...
  + This is also known as **moderation**, as the effect or association between two variables is being moderated by a third.
...
(p <- ggplot(intDat[intDat$int=='meInt',], aes(IV1, DV, color = IV2, group = IV2)) + geom_point(size = 3) + geom_line(linewidth = 1) +
...
```{r, echo = F, fig.height=6, fig.width=9}
p +
  annotate(geom = 'point', x = 1, y = 3, shape = 8, size = 4) +
  annotate(geom = 'point', x = 2, y = 3.5, shape = 8, size = 4) +
  annotate(geom = 'segment', x = 1, xend = 2, y = 3, yend = 3.5, linetype = 2) +
  annotate(geom = 'text', label = 'Difference between IV1 Groups A & B', x = 1.5, y = 3.52, size = 5) +
  annotate(geom = 'text', label = 'Mean of IV1 \n Group A', x = .85, y = 3, size = 5) + 
  annotate(geom = 'text', label = 'Mean of IV1 \n Group B', x = 2.2, y = 3.55, size = 5)
```
]
...
p +
  ...
  annotate(geom = 'text', label = 'Difference between \n IV2 Groups A & B', x = 1.74, y = 3.25, size = 5) +
  annotate(geom = 'text', label = 'Mean of IV2 \n Group A', x = 1.5, y = 4.4, size = 5) +
  annotate(geom = 'text', label = 'Mean of IV2 \n Group B', x = 1.5, y = 2.1, size = 5)
```
]
...
p + 
  annotate(geom = 'text', label = 'Difference between \n IV2 Groups at IV1A', x = .75, y = 3, size = 5) +
  annotate(geom = 'text', label = 'Difference between \n IV2 Groups at IV1B', x = 2.25, y = 3.5, size = 5) +
  annotate(geom = 'segment', x = 1, xend = 1, y = 2, yend = 4, lty = 2) +
  annotate(geom = 'segment', x = 2, xend = 2, y = 3, yend = 4, lty = 2)
...
+ Because there are multiple independent variables, there are also multiple possible hypotheses.

  + Hypotheses about main effects
  
  + Hypothesis about interaction

+ A key consideration when working with tests that produce multiple results is to be sure the research aims and hypotheses align with the focus in the results and discussion. 
...
(ssMod <- ggplot(sampDat, aes(intOrder, Recognition, color = intGroup)) + geom_point() +
    ...
    geom_segment(x = sampDat$intOrder, y = mean(sampDat$Recognition), xend = sampDat$intOrder, yend =
                   sampDat$intmean, lty = 2, linewidth = 1) +
...
(groupSS <- ggplot(sampDat, aes(GroupOrder, Recognition, color = GroupMatch)) + geom_point() +
  ...
  scale_x_continuous(breaks = seq(1, 18, 1)) +
  scale_y_continuous(limits = c(0, 50), breaks = seq(0, 50, 10)) + 
  labs(x = 'Participant ID', color = 'Stimulus Group') +
  geom_hline(yintercept = mean(sampDat$Recognition), linewidth = 1) +
...
```{r, echo = F, warning = F, fig.height=5, fig.width = 6}
ssMod + theme(legend.position = 'none') +
  xlab('Participant ID') +
  annotate(geom = 'text', label = 'SS Model (Variance in DV explained by IVs)', x = 9, y = 10, size = 5)
```
...
$SS_{IV1}$ (Coverage) and $SS_{IV2}$ (Group Membership) can be computed...
...
The interaction effect can then be computed: $SS_{IV1\times IV2} = SS_{model} - (SS_{IV1} + SS_{IV2})$
...
+ 3 hypotheses (2 main effects and 1 interaction):
  ...
  + RQ1 - Does facial coverage type affect recognition of facial expression?
      + $H_1$ Type of facial coverage has an effect on recognition of facial expression
  
  + RQ2 - Is there an effect of stimulus ethnicity on recognition of facial expression?
      + $H_1$ Participants are more likely to recognise facial expressions in members of their own ethnic group.
    
  + RQ3 - Does the effect of facial coverage on expression recognition change depending upon stimulus ethnicity?
      + $H_1$ The effect of facial coverage on expression recognition is dependent upon stimulus group.
...
+ Main Effects Hypotheses:
  ...
  + Main Effect of Stimulus Ethnicity:
...
<font size="3"> 
  ...
  $C = (\mu_{consistent}\times\mu_{none}-\mu_{consistent}\times\mu_{mask}-\mu_{consistent}\times\mu_{sunglasses}) - (\mu_{inconsistent}\times\mu_{none}-\mu_{inconsistent}\times\mu_{mask}-\mu_{inconsistent}\times\mu_{sunglasses})$ 
...
+ A power analysis should be conducted for each effect of interest

> **Test your Understanding:** How many power analyses should be run in this instance?
...
We conducted a **2x3 Between-Subjects Factorial ANOVA** to determine the effect of <span style = "color:#BB5A73"> <b> facial coverage (levels: none; mask; sunglasses) </span></b> and <span style = "color:#BB5A73"> <b> stimulus ethnicity (levels: in-group; out-group) </span></b> on <span style = "color:#BB5A73"> <b> facial expression recognition </span></b>. The $\alpha$ threshold was set at .05 for all analyses. An a priori power analysis indicated that 120 participants were sufficient to detect an interaction effect of $\eta^2$ = .08 with a power of 80% and an $\alpha$ threshold of .05.

Facial coverage had a significant effect on expression recognition, <span style = "color:#DFAA73"><b> $F$(2, 114) = 20.49, $p$ < .001, $\eta^2_p$ = .26 </span></b>. However, stimulus group was not associated with expression recognition, <span style = "color:#DFAA73"><b> $F$(1, 144) = 0.53, $p$ = .470, $\eta^2_p$ = .005 </span></b>. 
...
There was a significant interaction between facial coverage and stimulus ethnicity on expression recognition, <span style = "color:#DFAA73"><b> $F$(2, 114) = 5.79, $p$ = .004, $\eta^2_p$ = .09 </span></b> (see Figure 1). Simple effects analyses were conducted to probe the interaction. Significant effects were found in both in-group, ** $F$(2, 114) = 9.44, $p$ < .001, $\eta^2_p$ = .14**, and out-group, ** $F$(2, 114) = 16.85, $p$ < .001, $\eta^2$ = .23**, conditions. 

Participants who viewed stimuli whose ethnicity was consistent with their own recognised significantly more expressions in stimuli with no facial coverage than those wearing sunglasses, <span style = "color:#F0D264"> $p$ < .001. </span> Additionally, they recognised more expressions in those with masks than those with sunglasses, <span style = "color:#F0D264"> $p$ = .015. </span> There was no difference in recognition when stimuli were wearing masks and when they had no facial coverage, <span style = "color:#F0D264"> $p$ = .485. </span>

Participants viewing stimuli inconsistent with their ethnicity recognised significantly more expressions when stimuli had no facial coverage, as opposed to when they were wearing masks <span style = "color:#F0D264"> ( $p$ < .001) </span> or sunglasses <span style = "color:#F0D264"> ( $p$ < .001) </span>. There was no difference in recognition when stimuli were wearing masks versus when they were wearing sunglasses <span style = "color:#F0D264"> ( $p$ = .556) </span>.
