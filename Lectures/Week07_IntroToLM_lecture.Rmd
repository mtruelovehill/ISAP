...
.center[
```{r, echo = F, fig.show='hold', message = F, fig.width=5, fig.height=4}
...
.center[
```{r, echo = F, fig.show='hold', message = F, fig.width=5, fig.height=4.25}
...
ggplot() + 
  ...
  annotate(geom = 'segment', x = 2, xend = 2, y = 2, yend = 3, color = accent1, linewidth = 1) +
  annotate(geom = 'segment', x = 2, xend = 3, y = 3, yend = 3, color = accent1, linewidth = 1) +
  annotate(geom = 'point', x = 0, y = 0, color = accent1, size = 6, shape = 18) +
  annotate(geom = 'text', label = 'Intercept', x = .5, y = 0, color = accent1, size = 5) +
  annotate(geom = 'text', label = 'Slope', x = 2.5, y = 3.25, color = accent1, size = 5) +
...
$$y_i = \beta_0 + \beta_1\color{#BB5A73}{x_i} + \epsilon_i$$
.center[
```{r, echo = F, message = F, fig.height=5}
...
lrPlot + 
  ...
  annotate(geom = 'text', label = dat$labDat, x = dat$StudyHours + .15, y = dat$CourseAverage, color = baseColor, 
           size = 5, fontface = 'bold') +
  geom_segment(x = dat$StudyHours, xend = dat$StudyHours, y = 0, yend = dat$CourseAverage, color = accent2, linetype = 'dashed')
```
]]
...
$$\color{#BB5A73}{y_i} = \beta_0 + \beta_1x_i + \epsilon_i$$
.center[
```{r, echo = F, message = F, fig.height=5}
...
lrPlot + 
  ...
  annotate(geom = 'text', label = dat$yLabs, x = dat$StudyHours + .15, y = dat$CourseAverage, color = baseColor, 
           size = 5, fontface = 'bold') +
  geom_segment(x = 0, xend = dat$StudyHours, y = dat$CourseAverage, yend = dat$CourseAverage, color = accent2, linetype = 'dashed')
```
]]
...
$$y_i = \beta_0 + \beta_1x_i + \color{#BB5A73}{\epsilon_i}$$
...
$$\epsilon_i = y_i-\color{#BB5A73}{\hat{y_i}}$$
...
$$y_i = \color{#BB5A73}{\beta_0} + \beta_1x_i + \epsilon_i$$
.center[
...
$$y_i = \beta_0 + \color{#BB5A73}{\beta_1}x_i + \epsilon_i$$
.center[
...
+ Open [these data](https://mtruelovehill.github.io/ISAP/Data/courseDat.sav) in SPSS.
...
We conducted a **multiple linear regression** to determine the effect of <span style = "color:#CD826E"> <b> revision time (in hours per week) </span></b> and <span style = "color:#CD826E"> <b> overall programme average </span></b> on <span style = "color:#CD826E"> <b> course performance (measured as the course average) </span></b>. The $\alpha$ threshold was set at .05 for all analyses. An a priori power analysis indicated that 90 participants were sufficient to detect an effect of $R^2$ = .1 with a power of 80% and an $\alpha$ threshold of .05 using a model with two predictors.

Multiple linear regression indicated that revision time explained 17% ($R^2_{adj}$) of the variance in course performance <span style = "color:#BB5A73"><b> $F$(2, 118) = 13.16, $p$ < .001 </span></b>. For every additional hour of study time per week, the average course mark increased by 1.93 while controlling for overall programme performance, <span style = "color:#F0D264"><b>  $SE$ = .39, $p$ < .001, 95% CI = [1.16, 2.71] </span></b>. Overall programme performance did not significantly predict course performance, <span style = "color:#F0D264"><b> $\beta$ = .18, $SE$ = .10, $p$ = .069, 95% CI = [-0.01, 0.38] </span></b>. 
