...
+ Probability distributions provide information about the frequency of specific observations in a range of data

+ When we run a statistical test, we compare our results against the null distribution, which is the probability distribution of the expected results if the null hypothesis is true.
...
.pull-left[
+ A unimodal probability distribution where values around the mean are most common, and values further from the mean are increasingly less common.
...
normDat <- data.frame(x = rnorm(100))

ggplot(normDat, aes(x)) +
  stat_function(fun=dnorm, args= list(0, 1), geom = "line", linewidth = 1.5, color = accent2) +
  scale_x_continuous(limits = c(-5, 5)) +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank()) +
  geom_vline(xintercept = 0, color = accent2, linewidth = 1, linetype = 'dashed')
```
]
...
.pull-left[
+ In data that are perfectly normally distributed:
...
ggplot(normDat, aes(x)) +
  ...
  geom_area(stat = 'function', fun = dnorm, args= list(0, 1), fill = accent1, xlim=c(-1, 1), alpha = .8) +
  geom_vline(xintercept = 1, color = accent1, linewidth = 1) + 
  geom_vline(xintercept = -1, color = accent1, linewidth = 1) +
  annotate(geom = 'text', label = '68%', x = 0.05, y = .2, size = 6, color = 'white')
```
]
...
ggplot(normDat, aes(x)) +
  ...
  theme(axis.text.y = element_blank(),
        ...
        axis.text.x = element_text(size = 12),
        axis.title.x = element_text(size = 14, face = 'bold')) + 
  geom_area(stat = 'function', fun = dnorm, args= list(0, 1), fill = accent1, xlim=c(-1.96, 1.96), alpha = .8) +
  geom_vline(xintercept = 1.96, color = accent1, linewidth = 1) + 
  geom_vline(xintercept = -1.96, color = accent1, linewidth = 1) +
  annotate(geom = 'text', label = '95%', x = 0.05, y = .2, size = 6, color = 'white')
```
]
---
## Characteristics of the normal distribution
...
ggplot(normDat, aes(x)) +
  ...
  theme(axis.text.y = element_blank(),
        ...
        axis.text.x = element_text(size = 12),
        axis.title.x = element_text(size = 14, face = 'bold')) + 
  geom_area(stat = 'function', fun = dnorm, args= list(0, 1), fill = accent1, xlim=c(-3, 3), alpha = .8) +
  geom_vline(xintercept = 3, color = accent1, linewidth = 1) + 
  geom_vline(xintercept = -3, color = accent1, linewidth = 1) +
  annotate(geom = 'text', label = '99.7%', x = 0.05, y = .2, size = 6, color = 'white')
```
]
...
.pull-left[
Changing the **mean** impacts where the curve is centered on the $x$-axis.

```{r, echo = F, fig.height=3.5, fig.width = 6}
ggplot(normDat, aes(x)) +
  stat_function(fun=dnorm, args= list(0, 1), geom = "line", linewidth = 1.5, color = accent2) +
  stat_function(fun=dnorm, args= list(-2, 1), geom = "line", linewidth = 1.5, color = accent1) +
  stat_function(fun=dnorm, args= list(2, 1), geom = "line", linewidth = 1.5, color = accent3) +
  ...
  theme(axis.text = element_blank(),
        ...
        axis.title.x = element_text(size = 14, face = 'bold')) +
  geom_vline(xintercept = 0, color = accent2, linewidth = 1, linetype = 'dashed') +
  geom_vline(xintercept = -2, color = accent1, linewidth = 1, linetype = 'dashed') +
  geom_vline(xintercept = 2, color = accent3, linewidth = 1, linetype = 'dashed') +
  annotate(geom = 'text', label = 'mean = -2', x = -5, y = .35, color = accent1, size = 6, fontface = 2) +
  annotate(geom = 'text', label = 'mean = 0', x = -5, y = .3, color = accent2, size = 6, fontface = 2) +
  annotate(geom = 'text', label = 'mean = 2', x = -5, y = .25, color = accent3, size = 6, fontface = 2)
```

]

.pull-right[
Changing the **standard deviation** impacts the shape of the curve.

```{r, echo = F, fig.height=3.5, fig.width = 6}
ggplot(normDat, aes(x)) +
  stat_function(fun=dnorm, args= list(0, 0.5), geom = "line", linewidth = 1.5, color = accent1) +
  stat_function(fun=dnorm, args= list(0, 1), geom = "line", linewidth = 1.5, color = accent2) +
  stat_function(fun=dnorm, args= list(0, 2), geom = "line", linewidth = 1.5, color = accent3) +
  scale_x_continuous(limits = c(-6, 6)) +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title.y = element_blank(),
        axis.title.x = element_text(size = 14, face = 'bold')) +
  geom_vline(xintercept = 0, color = accent2, linewidth = 1, linetype = 'dashed') +
  annotate(geom = 'text', label = 'sd = 0.5', x = -4, y = .7, color = accent1, size = 6, fontface = 2) +
  annotate(geom = 'text', label = 'sd = 1', x = -4, y = .6, color = accent2, size = 6, fontface = 2) +
  annotate(geom = 'text', label = 'sd = 2', x = -4, y = .5, color = accent3, size = 6, fontface = 2)
```
]
---
## Effect Size
...
(p1 <- ggplot(dat, aes(x)) + geom_density(data = dat[dat$group=='A',], fill = accent1, alpha = .7) +
  geom_vline(xintercept = mean(dat$x[dat$group=='A']), color = accent1, linewidth = 1.5, linetype = 2) +
...
(p2 <- ggplot(dat2, aes(x)) + geom_density(data = dat2[dat2$group=='A',], fill = accent1, alpha = .7) +
  geom_vline(xintercept = mean(dat2$x[dat2$group=='A']), color = accent1, linewidth = 1.5, linetype = 2) +
...
p1 +
  annotate('segment', x=mean(dat$x[dat$group=='A']), xend = mean(dat$x[dat$group=='B']), y = .1, color = baseColor, linewidth = 1.5) +
  annotate('text', label = paste('effect size =', round(abs(dP1$effsize), 2), '(large)'), x=28, y = .1, color = baseColor, size = 5)
```
...
p2 +
  annotate('segment', x=mean(dat2$x[dat2$group=='A']), xend = mean(dat2$x[dat2$group=='B']), y = .15, color = baseColor, linewidth = 1.5) +
  annotate('text', label = paste('effect size =', round(abs(dP2$effsize), 2), '(small)'), x=24, y = .15, color = baseColor, size = 5)
```
...
+ A **confidence interval** defines a plausible range of values for the population parameter of interest.
  ...
  + (Remember, we don't actually know the population parameter; we're trying to estimate this with our sample's data)
...
ggplot(sampDat, aes(MeanScore)) + geom_histogram(binwidth = 2, fill = baseColor, color = accent2) +
...
.center[
```{r, echo = F, fig.height=3.5, warning = F}
ggplot(seDat, aes(MeanScore)) + geom_histogram(binwidth = 2, fill = baseColor, color = accent2) +
  scale_y_continuous(breaks=seq(0, 20, 5), limits = c(0, 20)) +
  scale_x_continuous(breaks=seq(45, 80, 5), limits = c(45, 80)) +
  xlab('Mean Score') +
  theme(axis.text = element_text(size = 12),
...
ggplot(seDat, aes(MeanScore)) + geom_histogram(binwidth = .75, fill = accent1, color = accent2) +
  scale_y_continuous(breaks=seq(0, 20, 5), limits = c(0, 20)) + 
  scale_x_continuous(breaks=seq(45, 80, 5), limits = c(45, 80)) +
  labs(x = 'Mean Score', y = 'Count') +
  theme(axis.text = element_text(size = 12),
...
ggplot(smallSE, aes(MeanScore)) + geom_histogram(binwidth = .75, fill = accent1, color = accent2) +
  scale_y_continuous(breaks=seq(0, 20, 5), limits = c(0, 20)) + 
  scale_x_continuous(breaks=seq(45, 80, 5), limits = c(45, 80)) +
  labs(x = 'Mean Score', y = 'Count') +
  theme(axis.text = element_text(size = 12),
...
ggplot(seDat, aes(MeanScore)) +
  stat_function(fun=dnorm, args= list(mean(seDat$MeanScore), sd(seDat$MeanScore)), geom = "line", linewidth = 1.5, color = accent1) +
  ...
  geom_vline(xintercept = loBand, color = accent3, linewidth = 1) +
  geom_vline(xintercept = hiBand, color = accent3, linewidth = 1) +
  geom_vline(xintercept = mean(seDat$MeanScore), color = accent1, linewidth = 1, linetype = 'dashed') +
...
+ In most a priori power analyses, you'll calculate the **sample** required to get 80% power for a specific effect size and $\alpha$ value.

+ However, if you already have a set sample (e.g. a secondary data analysis), you may want to compute the **effect size** that it can detect, given certain $\alpha$ and power thresholds.
